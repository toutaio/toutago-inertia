...
func TestAlways(t *testing.T) {
	...
	t.Run("always props included in partial reload", func(t *testing.T) {
		req := httptest.NewRequest("GET", "/users", http.NoBody)
		req.Header.Set("X-Inertia", "true")
		req.Header.Set("X-Inertia-Partial-Data", "name")
		req.Header.Set("X-Inertia-Partial-Component", "Users/Index")
		...
		// "auth" should always be included, even in partial reloads
		err := ic.Always("auth", map[string]interface{}{
			"user": "current_user",
		}).Render("Users/Index", props)
		require.NoError(t, err)

		// Check that response includes "auth" even though only "name" was requested
		// This is verified through the response body containing both "auth" and "name"
		assert.Contains(t, w.Body.String(), "auth")
		assert.Contains(t, w.Body.String(), "name")
	})

	t.Run("always props work with lazy evaluation", func(t *testing.T) {
		req := httptest.NewRequest("GET", "/users", http.NoBody)
		req.Header.Set("X-Inertia", "true")
		req.Header.Set("X-Inertia-Partial-Data", "name")
		req.Header.Set("X-Inertia-Partial-Component", "Users/Index")
		...
		err := ic.AlwaysLazy("auth", authFn).Render("Users/Index", props)
		require.NoError(t, err)

		// Auth should be evaluated even in partial reload
		assert.True(t, called, "always lazy prop should be evaluated")
		assert.Contains(t, w.Body.String(), "auth")
	})
}
